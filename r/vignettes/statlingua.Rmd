...
3.  **System Prompt Assembly (via `.assemble_sys_prompt()`)**:
    This is where **statlingua** constructs the detailed instructions for the LLM. The internal `.assemble_sys_prompt()` function pieces together several components from the package's `inst/prompts/` directory (generated from the canonical `prompts/` directory at the root of the [statlingua repository](https://github.com/bgreenwell/statlingua), shared with the Python package) and interpolates them into `inst/prompts/system_prompt_template.md` via [`ellmer::interpolate_package()`](https://ellmer.tidyverse.org/reference/interpolate.html). The final system prompt typically includes the following sections, ordered to guide the LLM effectively:
